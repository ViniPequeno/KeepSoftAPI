...
public class NotificacaoController {
    ...
    public ResponseEntity<Notificacao> inserirNotificacao(@Valid @RequestBody Notificacao notificacao) {
        ...
        try {
            notificacao.setDataCriacao(formato.parse(notificacao.getDataCriacaoFormat()));
    ...
    public ResponseEntity<Notificacao> atualizarNotificacao(@PathVariable(value = "id") Long notificacaoId,
        ...
        formato.setTimeZone(TimeZone.getTimeZone("GMT-4:00"));
        try {
            notificacao.setDataCriacao(formato.parse(notificacaoUpdate.getDataCriacaoFormat()));
        } catch (ParseException e) {
